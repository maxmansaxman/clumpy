...
# Theoretical equilibrium values for Δ47 in CO2 gas
# according to Wang et al. (2004)
# [http://dx.doi.org/10.1016/j.gca.2004.05.039],
# from supplementary data of Dennis et al. (2011)
...
eq_val = scipy.array([l.split('\t') for l in eq_val.split('\n')], dtype='float')
T = eq_val[:,0] - 0.15
D47 = eq_val[:,1]

# Quadratic polynomial fit as used by Dennis et al. (2011)
# [http://dx.doi.org/10.1016/j.gca.2011.09.025]:
eq_fit = scipy.polyfit( 1/(T+273.15), D47, 4 )
EqD47 = lambda T : \
	eq_fit[0] * (T+273.15)**-4 + \
	eq_fit[1] * (T+273.15)**-3 + \
	eq_fit[2] * (T+273.15)**-2 + \
...
# lines starting with 's' correspond to samples;
...
s		15.2	.095	.010	24	Sample A
s		13.0	-.054	.006	25	Sample B
s		9.50	-.225	.012	26	Sample C"""

# ms_data = """type	Teq	d47	D47	sD47	id	label
# e	7.035	1	+0.5	.010	1
# e	7.035	19	+0.5	.010	2
# e	1000	1	-0.5	.010	3
# s		10.0	0.00	.010	5	"""
...
# to straight lines with the same slope:
...
Drange = scipy.r_[scipy.ceil(Dmin*10)/10:scipy.ceil(Dmax*10)/10:.1]
...
pylab.xlabel(u'δ47 (‰)')
pylab.ylabel(u'Uncorrected Δ47 (‰)')
